...
if (taskid == MASTER) {
  ...
  for (dest=1; dest<= numworkers; dest++) {
    ...
    fflush(stdout); // Vuelve a vaciar el buffer
    MPI_Send(&index, 1, MPI_INT, dest, MASTER, MPI_COMM_WORLD);
    MPI_Send(&data[index], chunksize, MPI_FLOAT, dest, MASTER, MPI_COMM_WORLD);
  ...
  for (i=1; i<= numworkers; i++) {
    ...
    MPI_Recv(&result[index], chunksize, MPI_FLOAT, source, 1, MPI_COMM_WORLD, &status);
