...
static uint64_t _dofib1(uint64_t n) __attribute__((noinline));
 ...
 */
static uint64_t _dofib1(uint64_t n)
{
	if (n == 0)
		return 0;
	if (n == 1)
	...
	return _dofib1(n-1) + _dofib1(n-2);
}

/*
 * fake call to surround _dofib() with visible symbols
 */
uint64_t fake_fib1(uint64_t n)
{
	return _dofib1(n);
}
