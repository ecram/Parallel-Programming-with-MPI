 ...
 *         Solution program demonstrating that replacing a blocking receive
     ...
     char **argv;
{
  ...
  /* ------------------------------------------------------------------
   * task 1 calls a non-blocking receive, sleeps for 10 seconds, and
   * then waits for the receive to complete.
   * the sleep is intended to simulate time spent in useful computation
   * ------------------------------------------------------------------ */
  else if ( rank == 1)  {
    for (i=0; i<MSGLEN; i++) message[i] = -100;
    MPI_Irecv ( message, MSGLEN, MPI_FLOAT, 0, TAG, MPI_COMM_WORLD,
	       &request);
/*    MPI_Sleep(10); */
    sleep(10);
